package com.example.common.proxy.model;

import com.example.common.proxy.config.Translator;
import com.example.common.proxy.exception.BaseApiException;
...
public class ApiError {
	...
	public static ApiError buildApiError(Throwable throwable, HttpStatus httpStatus) {
		...
		if (throwable instanceof BaseApiException) {
			...
			return ApiError.builder().code(httpStatus.value()).message(Translator.toLocale("error.default"))
					.debugMessage(String.valueOf(throwable)).extraData(Collections.EMPTY_MAP).build();
		}
	}

	private static ApiError buildConflictApiError(Throwable e) {
		return ApiError
				.builder()
				.code(HttpStatus.CONFLICT.value())
				.message(e.getMessage())
				.debugMessage(e.toString())
				.extraData(Collections.EMPTY_MAP).build();
	}
	...
	private static ApiError buildApiError(ConstraintViolationException e) {
		...
		return ApiError.builder().code(HttpStatus.BAD_REQUEST.value())
				.message(Translator.toLocale(messageCode, defaultMessage)).debugMessage(e.toString())
				.extraData(Collections.EMPTY_MAP).build();
	}
	...
	private static ApiError buildApiError(BaseApiException e) {
		return ApiError.builder().code(e.code())
				.message(StringUtils.defaultIfBlank(e.displayMessage(), Translator.toLocale("error.default")))
				.debugMessage(e.toString()).extraData(e.extraData()).build();
	}
